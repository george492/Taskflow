...
function UpdateTask() {
  ...
  useEffect(() => {
    ...
    if (task.status === "Completed") {
      //  task.TaskDone=parseFloat(task.totalTasks);
      console.log("task.completed");
      setRate(true);
      console.log("task.rate", rate);
      //  task.TaskDone=parseFloat(task.TaskDone);
    }
    else if (parseFloat(task.TaskDone) === parseFloat(task.totalTasks)) {
  ...
  }, [task.TaskDone, task.totalTasks, task.status])
  ...
  const handleUpdate = async () => {
    ...
    else if (parseFloat(task.TaskDone) < 0) {
      setError_neg(true);
      setTimeout(() => {
        setError_neg(false);
      }, 3000);
    ...
    try {
      const updatedTask = {
        ...
        TaskDone: task.status === "Completed" ? task.totalTasks : task.TaskDone,
        totalTasks: task.totalTasks,
        Checklist: task.checklist,
        deadline: task.deadline,
        ratingGiven: task.ratingGiven > 5 ? 5 : task.ratingGiven || task.ratingGiven < 0 ? 0 : task.ratingGiven,
    ...
    React.createElement('div', { className: 'row' },
      React.createElement('div', { className: 'col' },
        ...
        React.createElement('input', {
          ...
          value: task.TaskDone,
          onChange: (e) => {
            setTask({ ...task, TaskDone: e.target.value })
